...
def train(learning_rate =0.0001, nums_epoch = 1500, minibatch_size = 32, print_cost = True):
    ...
    for epoch in range(1500):
        ...
        for minibatch in minibatches:
            ...
            train_acc += correct_prediction.int()
            epoch_accracy += 1.0*correct_prediction / minibatch_size / len(minibatches)
            epoch_cost += minibatch_cost / len(minibatches)

        if print_cost == True and epoch % 100 ==0:
            print(train_acc)
            print ("Cost after epoch %i: %f" % (epoch, epoch_cost))
            print ("Traing Acc. after epoch %i: %f" % (epoch, train_acc.numpy()/Y_train.shape[1]))
            ...
            test_acc = correct_prediction.int()
            #correct_prediction = sum(torch.max(test_output) == torch.max(Y_test_tensor))
            correct_prediction = correct_prediction*1.0 / test_output.size(0)
            print ("test Acc. after epoch %i: %f" % (epoch, test_acc.numpy()/120))
